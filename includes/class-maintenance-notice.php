...
if ( !class_exists( 'Maintenance_Notice' ) ):
    ...
    class Maintenance_Notice {
        ...
        function enqueue_scripts() {
            ...
            wp_enqueue_style( 'jquery-YTPlayer', esc_url( MAINTENANCE_NOTICE_INCLUDES_URL . '/assets/library/jquery.mb.YTPlayer/jquery.mb.YTPlayer.min.css' ), array(), MAINTENANCE_NOTICE_VERSION, 'all' );
            
            wp_enqueue_style( 'slick-slider', esc_url( MAINTENANCE_NOTICE_INCLUDES_URL . '/assets/library/slick/slick.css' ), array(), '1.8.0', 'all' );

            wp_enqueue_style( 'font-awesome', esc_url( MAINTENANCE_NOTICE_URL . '/admin/assets/library/font-awesome/css/all.min.css' ), array(), '6.5.1', 'all' );

            wp_enqueue_style( 'crossfade', esc_url( MAINTENANCE_NOTICE_INCLUDES_URL . '/assets/library/crossfadeimage/main.css' ), array(), MAINTENANCE_NOTICE_VERSION, 'all' );

            wp_enqueue_style( 'cvmn-frontpage-style', esc_url( MAINTENANCE_NOTICE_INCLUDES_URL . '/assets/css/style.css' ), array(),
                MAINTENANCE_NOTICE_VERSION, 'all' );

            wp_enqueue_script( 'jquery-countdown', esc_url( MAINTENANCE_NOTICE_INCLUDES_URL . '/assets/library/Minimal-jQuery-Countdown/jquery.countdown.min.js' ), array(), '1.0.2', true );

            wp_enqueue_script( 'slick-slider', esc_url( MAINTENANCE_NOTICE_INCLUDES_URL . '/assets/library/slick/slick.js' ), array(), '1.8.0', true );

            wp_enqueue_script( 'cvmn-frontpage-script', esc_url( MAINTENANCE_NOTICE_INCLUDES_URL . '/assets/js/main.js' ), array( 'jquery' ),
                MAINTENANCE_NOTICE_VERSION, true );
            ...
            if ( isset( $maintenance_notice_options["cvmn_maintenance_page_background_type"] ) ) {
                if ( $maintenance_notice_options["cvmn_maintenance_page_background_type"] === 'video-background' ) {
                    $bgVideo = 'true';
                    wp_enqueue_script( 'jquery-YTPlayer', esc_url( MAINTENANCE_NOTICE_INCLUDES_URL . '/assets/library/jquery.mb.YTPlayer/jquery.mb.YTPlayer.min.js' ), array(), MAINTENANCE_NOTICE_VERSION, true );
                }
            }

            wp_localize_script( 'cvmn-frontpage-script', 'cvmnObject',
                array(
                    'bgVideo'       => $bgVideo
                )
            );
    ...
    ?>
            ...
            <style id="cvmn-frontpage-inline-style">
                <?php
                    ...
                    echo wp_kses_post( $maintenance_notice_dynamic_css->generate_dynamic_css() );
                ?>
            </style>
    <?php
        }
