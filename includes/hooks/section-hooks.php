...
?>
        <header class="page-header">
            <?php
                if ( !empty( $cvmn_logo ) ) {
            ?>
                    <img src="<?php echo esc_url( $cvmn_logo ); ?>" alt="<?php esc_attr_e( 'Maintenance Notice', 'maintenance-notice' ); ?>" class="cvmn-header-logo">
            <?php
                }
            ?>
            <h1 class="page-title"><?php echo esc_html( $cvmn_page_title ); ?></h1>
        </header><!-- .page-header -->
<?php
    }
...
?>
        <div class="page-heading">
            <?php echo esc_html( $cvmn_page_heading ); ?>
        </div>
<?php
    }
...
?>
        <div class="cvmn-footer-content"><?php echo esc_html( apply_filters( 'cvmn_copyright_text', sprintf( /* translators: %1$s: copyright symbol, %2$s: a current year */ esc_html__( 'copyright %1$s codevibrant %2$s', 'maintenance-notice' ), '&copy;', date( 'Y' ) ) ) ); ?></div>
<?php
    }
...
if ( !function_exists( 'cvmn_countdown_section' ) ) :
    ...
    function cvmn_countdown_section() {
        ...
        extract( $maintenance_notice_options );
    ?>
        <ul class="cvmn-countdown-content" data-date="<?php echo esc_attr( $cvmn_countdown_end_date ); ?>" data-time="<?php echo esc_attr( $cvmn_countdown_end_time ); ?>">
