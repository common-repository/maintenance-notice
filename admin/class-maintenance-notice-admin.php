...
if ( !class_exists( 'Maintenance_Notice_Admin' ) ) :
    ...
    class Maintenance_Notice_Admin {
        ...
        public function admin_enqueue_scripts( $hook_suffix ) {
            ...
            wp_enqueue_style( 'font-awesome', plugins_url( '/assets/library/font-awesome/css/all.min.css', __FILE__ ), array(), '6.5.1', 'all' );

            wp_enqueue_style( 'maintenance-notice-admin-style', plugins_url( 'css/admin.css', __FILE__ ), array(), MAINTENANCE_NOTICE_VERSION, 'all' );
            
            wp_enqueue_style( 'maintenance-notice-admin-icons-style', plugins_url( 'assets/cvmn-icons/style.css', __FILE__ ), array(), MAINTENANCE_NOTICE_VERSION, 'all' );

            wp_enqueue_media();

            wp_enqueue_script( 'maintenance-notice-admins-script', plugins_url( 'js/admin.js', __FILE__ ), array( 'jquery', 'wp-color-picker' ), MAINTENANCE_NOTICE_VERSION, true );

            wp_localize_script( 'maintenance-notice-admins-script', 'MaintenanceNoticeObject',
                array(
                ...
                <header id="cvmn-main-header" class="cvmn-tab-block-wrapper">
                    <div class="admin-main-menu nav-tab-wrapper cvmn-nav-tab-wrapper">
                        ...
                        <?php
                            $header_titles = array(
                                "dashboard" => array(
                                    "desc" => esc_html__( 'Get started!!', 'maintenance-notice' ),
                                    "icon" => 'cvicon-item cvicon-dashboard'
                                ),
                                "help"      => array(
                                    "desc" => esc_html__( 'Have an issue?', 'maintenance-notice' ),
                                    "icon" => 'cvicon-item cvicon-support'
                                ),
                                "review"    => array(
                                    "desc" => esc_html__( 'Review our product', 'maintenance-notice' ),
                                    "icon" => 'cvicon-item cvicon-review'
                                )
                            );
                            foreach( $header_titles as $header_title => $header_title_val ) {
                        ?>
                                <li class="nav-tab cvmn-nav-tab <?php echo 'cvmn-'.esc_attr( $header_title ); if ( $header_title == 'dashboard' ) { echo 'isActive'; } ?>">
                                    <a href="javascript:void(0)" data-tabId="<?php echo '#cvmn-'.esc_attr( $header_title ); ?>"><?php echo esc_html( str_replace( '-', ' ', $header_title ) ); ?>
        ...
        ?>
            <footer id="cvmn-main-footer" class="cvmn-promo-sidebar">
                ...
                <ul>
                    <li><a href="https://wpallresources.com/" target="_blank" title="<?php esc_attr_e( 'WP All Resources', 'maintenance-notice' ); ?>"> <?php echo '<img src="'.esc_url( plugins_url( 'maintenance-notice/includes/assets/images/wpallresources-img.png', dirname(__DIR__) ) ).'">'; ?></a></li>
                </ul>
                <h2><?php esc_html_e( 'Other Recommended Plugins', 'maintenance-notice' ); ?></h2>
                <ul>
                    <li><a href="https://wordpress.org/plugins/wp-blog-post-layouts/" target="_blank" title="<?php esc_attr_e( 'WP Blog Post Layouts', 'maintenance-notice' ); ?>"><?php echo '<img src="'.esc_url( plugins_url( 'maintenance-notice/includes/assets/images/blog-post-layouts-img.png', dirname(__DIR__) ) ).'">'; ?> </a></li>
                    <li><a href="https://codevibrant.com/wp-plugin/wp-magazine-modules-for-gutenberg-elementor/" target="_blank" title="<?php esc_attr_e( 'WP Magazine Modules Lite', 'maintenance-notice' ); ?>"><?php echo '<img src="'.esc_url( plugins_url( 'maintenance-notice/includes/assets/images/wp-magazine-module-img.png', dirname(__DIR__) ) ).'">'; ?></a></li>
                </ul>
        ...
        function get_font_variant() {
            ...
            foreach( $variant_array as $key => $variant ) {
                ...
                $variant_option_array .= '<option value="' .esc_attr( $variant ). '" ' .esc_attr( $variant_selected ). '>' .esc_html( $variant ). '</option>';
            }
        ...
        function default_values() {
            $default_values = array(
                'cvmn_maintenance_page_display'         => 'hide',
                'cvmn_maintenance_page_background_type' => 'none',
                ...
                'cvmn_background_overlay_type'          => 'none',
        ...
        function font_awesome_social_icon_array() {
            ...
            $social_icon_array = array( "fab fa-tumblr-square","fab fa-tumblr","fab fa-facebook-square","fab fa-facebook-messenger","fab fa-facebook-f","fab fa-facebook","fab fa-linkedin-in","fab fa-linkedin","fab fa-instagram","fab fa-pinterest-square","fab fa-pinterest-p","fab fa-pinterest","fab fa-whatsapp-square","fab fa-whatsapp","fab fa-tiktok","fab fa-twitter-square","fab fa-twitter","fab fa-x-twitter","fab fa-flickr","fab fa-snapchat-square","fab fa-snapchat-ghost","fab fa-spotify","fab fa-snapchat","fab fa-viber","fab fa-digg","fab fa-yelp","fab fa-scribd","fab fa-stumbleupon-circle","fab fa-stumbleupon","fab fa-reddit-square","fab fa-reddit-alien","fab fa-reddit","fab fa-vine","fab fa-vk","fab fa-xing-square","fab fa-xing", "fab fa-youtube-square","fab fa-youtube","fab fa-mix","fab fa-quora","fab fa-meetup","fab fa-twitch","fab fa-skype","fab fa-soundcloud","fab fa-qq","fab fa-line","fab fa-telegram-plane","fab fa-telegram","fab fa-foursquare","fab fa-ravelry","fab fa-deviantart", "fab fa-vimeo-v", );
